...
% Steve Kuznicki
function [datapts, numpts] = PX4ReadLog(dataFile)
%%
datapts = 0;
numpts = 0;
...
if exist([pwd,'/',dataFile],'file')
    dataFile=[pwd,'/',dataFile];
end

if ~ exist(dataFile,'file')
    disp('Error: Failed to find file ',dataFile);
    return
end

if ~ endsWith(dataFile,'.ulg') && ~ endsWith(dataFile,'.bin')
    disp('Error: Wrong file format, *.ulg or *.bin file is required.');
    return
end
